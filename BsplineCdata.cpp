...
BsplineCdata::BsplineCdata(const Cdata& data) : cdat(data), b(), nseg(0), ord(0) 
{
	computeBsplineInterp();
}

BsplineCdata::BsplineCdata(const Cdata& data, int seg, int order) : cdat(data), b(), nseg(seg), ord(order) 
{
	computeBsplineLeastSq(nseg,ord);
}
...
void BsplineCdata::computeBsplineInterp()
{
	...
	b = BsplineCurve(knots[3], knots[knots.getSize() - 4], 4, knots.getSize() - 4, cpoints, knots);
}


// computes the least squares BsplineCurve, consisting of nseg segments and order given by ord, that approximates the data set
void BsplineCdata::computeBsplineLeastSq(int nseg, int ord)
{
	// TO DO

	// 1. declare a Matrix<double> named mat and initiase with the result of calling the method computeLeastSquaresMatrix()
	Matrix<double> mat = computeLeastSquaresMatrix(nseg, ord);

	// 2. declare a Vector<Point3D> named rhs and initialise with the result of calling the method computeRHSVectorLeastSq()
	Vector<Point3D> rhs = computeRHSVectorLeastSq(nseg, ord);

	// 3 declare a LinearSolve object named lsolve and initialise with mat and rhs from 1 and 2
	LinearSolve<Point3D> lsolve(mat, rhs, rhs.getSize());

	// 4. declare a Vector<Point3D> named cpoints and initialise with the result of calling getSolution() for lsolve
	Vector<Point3D> coeffs = lsolve.getSolution();

	// 5. declare a Vector<double> object named param and initialise it with the result of calling getParam on cdat
	Vector<double> param = cdat.getParam();

	// 6. declare a Vector<double> object named temp of size nseg+1 and assign the values according to temp[i] = param[0] + i*(param[cdat.getNum()-1]-param[0])/nseg;
	Vector<double> temp(nseg + 1);
	for (int i=0; i<nseg+1; i++) temp[i] = param[0] + i * (param[cdat.getNum() - 1] - param[0]) / nseg;

	// 7. declare a Vector<double> named knots and initialise with the result of calling computeKnots(temp, nseg+1,ord)
	Vector<double> knots = computeKnots(temp, nseg + 1, ord);

	// 8. assign the data member b to the result of calling the BsplineCurve constructor : BsplineCurve(knots[ord-1], knots[knots.size() - ord], ord, nseg-1+ord, cpoints, knots)
	b = BsplineCurve(knots[ord - 1], knots[knots.getSize() - ord], ord, nseg - 1 + ord, coeffs, knots);
}

double BsplineCdata::computeLSQError() const
{
	//  The Bspline approximation is stored in the Bspline curve b 
	//  The data points and the parameter values are stored in the cdat object

	// 1. Using the cdat object get the data points and store in a Vector<Point3D> object called pts
	Vector<Point3D> pts = cdat.getPoints();

	// 2. Using the cdat object get the parameter values and store in a Vector<double> object called param
	Vector<double> param = cdat.getParam();

	// 3. Using these and the Bspline object b implement the loop to compute and return the error in the approximation
	double sum = 0.0;
	for (int i = 0; i < pts.getSize(); i++) {
		double dist = (pts[i].getX() - b(param[i]).getX()) * (pts[i].getX() - b(param[i]).getX()) +
			(pts[i].getY() - b(param[i]).getY()) * (pts[i].getY() - b(param[i]).getY()) +
			(pts[i].getZ() - b(param[i]).getZ()) * (pts[i].getZ() - b(param[i]).getZ());
		sum = sum + dist;
	}
	sum = sum / pts.getSize();
	return sqrt(sum);
...
double BsplineCdata::computeAVError() const
{
	//  The Bspline approximation is stored in the Bspline curve b 
	//  The data points and the parameter values are stored in the cdat object
	...
	for (int i = 0; i < pts.getSize(); i++) {
		double dist = sqrt(
			(pts[i].getX() - b(param[i]).getX()) * (pts[i].getX() - b(param[i]).getX()) +
			(pts[i].getY() - b(param[i]).getY()) * (pts[i].getY() - b(param[i]).getY()) +
			(pts[i].getZ() - b(param[i]).getZ()) * (pts[i].getZ() - b(param[i]).getZ())
		);
		sum += dist;
	}
	return sum / pts.getSize(); // Return the average error
}

double BsplineCdata::computeMAXError() const
{
	//  The Bspline approximation is stored in the Bspline curve b 
	//  The data points and the parameter values are stored in the cdat object
