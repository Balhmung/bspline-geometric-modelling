...
public:
	...
	*/
	...
	Vector<T> getRow(int i) const; // get the ith row
	Vector<T> getCol(int j) const; // get the jth column
 ...
 */
template <typename T>
Matrix<T>::Matrix(const Matrix<T>& m) : std::vector<std::vector<T> >()
{
...
*/
template<typename T>
Vector<T> Matrix<T>::getRow(int i) const // get the ith row
{
	int ncols = getNcols();
	Vector<T> v(ncols);

	for (int j = 0; j<ncols; j++) v[j] = (*this)[i][j];
	return v;
}
...
*/
template<typename T>
Vector<T> Matrix<T>::getCol(int j) const // get the jth column
{
	int nrows = getNrows();
	Vector<T> v(nrows);
	for (int i = 0; i<nrows; i++) v[i] = (*this)[i][j];
	return v;
}
...
std::ifstream& operator>>(std::ifstream& ifs, Matrix<T>& m) {
    ...
    // input the elements
    for (int i=0; i<m.getNrows(); i++) 
		for (int j = 0; j < m.getNcols(); j++) ifs >> m[i][j];
